...
class Expediente
{
    ...
    public function obtenerPorPaciente($paciente_id)
    {
        ...
        $stmt = $this->db->prepare("SELECT h.id, h.id_paciente, p.nombre, p.apellido, p.telefono, h.diagnostico, h.tratamiento, h.notas, h.fecha FROM historial_medico AS h
        ...
        $stmt = $this->db->prepare("SELECT p.nombre, p.apellido, p.telefono, d.nombre AS doctor, d.especialidad, r.medicamentos, r.dosis, r.indicaciones, h.diagnostico, r.fecha FROM recetas AS r
        ...
        $stmt = $this->db->prepare("SELECT ct.id, p.id AS id_cliente, p.nombre, p.apellido, p.email, p.telefono, ct.fecha_hora, ct.estado FROM citas AS ct
